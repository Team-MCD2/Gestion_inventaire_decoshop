-- ════════════════════════════════════════════════════════════════════════════
--  Marché de Mo' — Schéma Supabase / PostgreSQL — v2.0 (mai 2026)
-- ════════════════════════════════════════════════════════════════════════════
--
...
--    vide OU sur une base déjà peuplée (y compris une base ex-DECO SHOP avec
--    les colonnes couleur/taille/categorie). Aucune donnée n'est jamais détruite.
--    Toutes les commandes utilisent IF NOT EXISTS / IF EXISTS.
--
--  ▸ MIGRATION DEPUIS L'ANCIEN SCHÉMA DECO SHOP
--    Si tu rejoues ce script sur une base ex-DECO SHOP, il :
--      - Ajoute les colonnes grocery : rayon, format, dlc, magasin.
--      - Recopie categorie → rayon (sous forme de slug normalisé).
--      - Recopie taille → format pour les cas où c'était un poids/volume.
--      - Supprime ensuite couleur, taille, categorie, modele (obsolètes).
--    Les numéros d'article au format DECO-YYMMDD-NNNNNN sont CONSERVÉS tels
--    quels — seules les NOUVELLES créations utilisent MDM-YYMMDD-NNNNNN.
--
...
create table if not exists public.articles (
  ...
  -- Champs descriptifs grocery
  description          text           not null default '',
  marque               text           not null default '',
  rayon                text           not null default '',  -- slug : fruits-legumes, boucherie-halal, saveurs-asie…
  format               text           not null default '',  -- ex: 500g, 1L, 12 unités
  code_barres          text           not null default '',
  dlc                  date           null,                 -- date limite de consommation
  magasin              text           not null default '',  -- portet | toulouse-sud | tous

  -- Photo (data URL base64 ou URL externe Cloudinary)
...
alter table public.articles add column if not exists rayon              text           not null default '';
alter table public.articles add column if not exists format             text           not null default '';
alter table public.articles add column if not exists code_barres        text           not null default '';
alter table public.articles add column if not exists dlc                date           null;
alter table public.articles add column if not exists magasin            text           not null default '';
...
-- 2.2 Migration des données ex-DECO SHOP vers le schéma grocery
--     - categorie → rayon (slugifié)
--     - taille → format (transfère le contenu si format est vide)
--     On n'écrase JAMAIS un rayon/format déjà rempli.
do $$
begin
  if exists (select 1 from information_schema.columns
             where table_schema='public' and table_name='articles' and column_name='categorie') then
    update public.articles
       set rayon = lower(regexp_replace(
                     translate(categorie,
                              'ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝàáâãäåæçèéêëìíîïðñòóôõöøùúûüýÿœŒ',
                              'AAAAAACEEEEIIIIDNOOOOOOUUUUYaaaaaaceeeeiiiidnoooooouuuuyyoeOE'),
                     '[^a-zA-Z0-9]+', '-', 'g'))
     where rayon = '' and categorie is not null and categorie <> '';
  end if;

  if exists (select 1 from information_schema.columns
             where table_schema='public' and table_name='articles' and column_name='taille') then
    update public.articles
       set format = taille
     where format = '' and taille is not null and taille <> '';
  end if;
end $$;

-- 2.3 Supprimer les colonnes obsolètes (l'application ne les utilise plus)
alter table public.articles drop column if exists couleur;
alter table public.articles drop column if exists taille;
alter table public.articles drop column if exists categorie;
...
begin
  ...
  -- Magasin parmi les 3 valeurs autorisées (vide = "non précisé", toléré)
  if not exists (
    select 1 from pg_constraint
    where conname = 'articles_magasin_enum'
  ) then
    alter table public.articles
      add constraint articles_magasin_enum
      check (magasin in ('', 'portet', 'toulouse-sud', 'tous'));
  end if;
end $$;
...
create index if not exists idx_articles_rayon        on public.articles (rayon);
...
create index if not exists idx_articles_dlc          on public.articles (dlc);
create index if not exists idx_articles_magasin      on public.articles (magasin);

-- Anciens index décor à supprimer (idempotent)
drop index if exists public.idx_articles_categorie;
...
comment on table  public.articles                    is 'Catalogue des articles Marché de Mo''. Une ligne = un SKU.';
comment on column public.articles.id                 is 'Identifiant interne (UUID v4 ou MDM-YYMMDD-NNNNNN — généré côté app).';
comment on column public.articles.numero_article     is 'Numéro d''article public, format MDM-YYMMDD-NNNNNN. Les anciens numéros DECO-* sont conservés tels quels.';
comment on column public.articles.nom_produit        is 'Nom du produit / Libellé court (ex: Banane plantain, Pâte d''arachide).';
comment on column public.articles.description        is 'Description courte (1 à 2 phrases).';
comment on column public.articles.marque             is 'Marque du produit (ex: Maggi, Yobé, sans-marque).';
comment on column public.articles.rayon              is 'Rayon Marché de Mo'' (slug) : fruits-legumes, boucherie-halal, epices-du-monde, saveurs-afrique, saveurs-asie, saveur-mediterranee, saveur-sud-amer, balkans-turques, produits-courants, surgeles.';
comment on column public.articles.format             is 'Format / poids / contenance (ex: ''500g'', ''1L'', ''12 unités'', ''200ml'').';
comment on column public.articles.code_barres        is 'EAN-13 / UPC / GTIN — peut être vide.';
comment on column public.articles.dlc                is 'Date limite de consommation (DLC). Nullable pour les non-alimentaires.';
comment on column public.articles.magasin            is 'Magasin : ''portet'', ''toulouse-sud'', ''tous'' (présent dans les 2), ou '''' (non précisé).';
comment on column public.articles.photo_url          is 'Photo : data URL base64 (image/jpeg) ou URL externe (Cloudinary recommandé).';
...
select
  ...
  case
    when a.dlc is null                         then 'no_dlc'
    when a.dlc <  current_date                 then 'expired'
    when a.dlc <= current_date + interval '7 days' then 'expiring_soon'
    else                                            'ok'
  end as dlc_status
from public.articles a;

comment on view public.articles_with_status is
  'Vue lecture seule incluant le statut stock (en_stock / stock_faible / rupture) et le statut DLC (ok / expiring_soon / expired / no_dlc). À utiliser pour les requêtes SQL ad-hoc — l''application calcule ces champs côté JS dans src/lib/db.js.';
